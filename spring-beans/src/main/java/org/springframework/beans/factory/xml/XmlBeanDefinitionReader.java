...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	/** 自动检测验证模式的工作委托给了专门处理类 */
	private final XmlValidationModeDetector validationModeDetector = new XmlValidationModeDetector();

	/** 获取当前线程，正在加载的EncodedResource集合*/
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			...
			try {
				//从 encodedResource 中获取已经初始化好的 Resource对象，并获取其中的 inputStream
				InputSource inputSource = new InputSource(inputStream);
				if (encodedResource.getEncoding() != null) {
					inputSource.setEncoding(encodedResource.getEncoding());
				}
				//执行加载 BeanDefinitition
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//1. doLoadDocument.getValidationModeForResource(resource) 获取对XML文件的验证模式
			//2. doLoadDocument 加载XML文件,等到对应的Document
			Document doc = doLoadDocument(inputSource, resource);
			//3. registerBeanDefinitions 返回Document注册的Bean信息.
	...
	protected int detectValidationMode(Resource resource) {
		//不可读,抛出 BeanDefinitionStoreException
		if (resource.isOpen()) {
			throw new BeanDefinitionStoreException(
	 ...
	 * @see DefaultBeanDefinitionDocumentReader#doRegisterBeanDefinitions
	 */
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//在实例化 BeanDefinition 时候会将 BeanDefinitionCount(),默认使用继承自DefaultListableBeanFactory 的子类
