...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 *  检查缓存中是否存在实例 beanName -> bean instance
	 **/
	 ...
	 *  保存 BeanName和创建bean实例之间的关系 beanName -> bean instance
	 *  与singletonObject的不同在于，当一个 bean 被放入这里之后,那么当bean还在创建过程之后，就可以通过getBean方法获取到了，用来检测循坏引用
	 **/
		...
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			...
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {
					// 当某些方法需要提前初始化的时候，会调用 addSingleFactory 方法
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				//记录加载状态，也就是通过 this.singletonsCurrentlyInCreation.add(beanName)
				//将当前正要创建的bean在缓存中，这样便可以对循环依赖进行检测。
				...
				try {
					...
					//当 bean加载结束后需要移除缓存中对该 bean 的正在加载
					//状态的记录。
	 ...
	 *
	 *  判断该 bean 是否在创建中 。
	 *  在 Spring中，会有个专门的属性默认为 DefaultSingletonBeanRegistry的singletonsCurrentlyInCreation来记录 bean 的加载状态，
